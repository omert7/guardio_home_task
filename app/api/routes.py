...
from app.core.rules import find_all_matching_rules, Rule
...
class PokemonValidator:
    """Class for validating and parsing Pokemon data."""
...
class RequestForwarder:
    """Class for forwarding requests to downstream services."""
    ...
    @staticmethod
    async def forward_to_multiple_destinations(
        destinations: List[Rule], 
        data: Dict[str, Any], 
...
class PokemonStreamHandler:
    """Main handler for Pokemon stream requests."""
    
    def __init__(self):
        self.validator = PokemonValidator()
        self.forwarder = RequestForwarder()
    
    async def process_stream_request(self, request: Request, config: Config) -> Dict[str, Any]:
        ...
        try:
            ...
            body = await self.validator.validate_signature(request, secret)
            
            # Parse and validate the Pokemon data
            pokemon_json = await self.validator.parse_pokemon_data(body)
            pokemon_model = await self.validator.validate_pokemon_model(pokemon_json)
            ...
            logger.info(f"Received Pokemon: {pokemon_model.name} (#{pokemon_model.number}), " +
                        ...
                        f"Special: {pokemon_model.special_attack}/{pokemon_model.special_defense}, " +
                        f"Speed: {pokemon_model.speed}, " +
            ...
            base_headers = await self.forwarder.prepare_outgoing_headers(request, "Multiple Rules")
            ...
            try:
                ...
                all_responses = await self.forwarder.forward_to_multiple_destinations(
                    matched_rules, 
...
class TestDestinationHandler:
    """Handler for test destination endpoints."""
    
    @staticmethod
    async def handle_test_destination(request: Request) -> Dict[str, Any]:
        """Handle requests to the test destination endpoint.
        
        Args:
            request: The incoming request
            
        Returns:
            Dict: A response confirming receipt
        """
        try:
            ...
            try:
                ...
                # Log the full Pokemon data for debugging
                logger.info("TEST DESTINATION RECEIVED POKEMON:")
                logger.info(f"  Name: {body.get('name')}")
                logger.info(f"  Number: {body.get('number')}")
                logger.info(f"  Type: {body.get('type_one')}/{body.get('type_two')}")
                logger.info(f"  Stats: HP={body.get('hit_points')}, ATK={body.get('attack')}, DEF={body.get('defense')}, SPD={body.get('speed')}")
                logger.info(f"  Legendary: {body.get('legendary')}")
    ...
    @staticmethod
    async def handle_test_destination_2(request: Request) -> Dict[str, Any]:
        """Handle requests to the second test destination endpoint.
...
# Initialize handlers
stream_handler = PokemonStreamHandler()
test_destination_handler = TestDestinationHandler()
...
async def stream(request: Request, config: Config = Depends(get_config)):
    ...
    """
    return await stream_handler.process_stream_request(request, config)
...
async def test_destination(request: Request):
    ...
    """
    return await test_destination_handler.handle_test_destination(request)
...
async def test_destination_2(request: Request):
    ...
    Returns:
        Dict: A response with a different format from the first test endpoint
    """
    return await test_destination_handler.handle_test_destination_2(request)
